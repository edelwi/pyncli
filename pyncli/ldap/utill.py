...
GENERALIZED_TIME_PATTERN=re.compile(
    r"^(?P<year>\d{4})(?P<month>\d{2})(?P<day>\d{2})(?P<hour>\d{2})(?P<minute>\d{2})(?P<second>\d{2}).0Z$", re.IGNORECASE)
...
def download_file(url, local_path, user, pwd):
    ...
    if r.status_code!=200:
        raise admexept.ConnectionFailure('File Download Error, status code: %s' % r.status_code)
...
def upload_file(url,data,login,pwd):
    ...
    if r.status_code!=200:
        raise admexept.OperationFailure('The server cannot complete the request. Code: %s <%s>' % (r.status_code, r.text))
        ...
        for ptr in (DATE_STR_PATTERN, DATE_STR_PATTERN2):
            ...
            if date_time:
                ...
                try:
                    ...
                    raise admexept.WrongParam(u"Invalid date format ({date}).".format(date=date_str))
                return u'{year}{month:02d}{day:02d}000000.0Z'.format(
                        year=py_time.year,
                        month=py_time.month,
                        day=py_time.day)

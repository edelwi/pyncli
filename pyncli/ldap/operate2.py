...
from . import group
from . import ou
from .admexept import AdminException, ConnectionFailure, \
    EmptyParam, NotEnoughParams, OperationFailure, TooLong, WrongParam, NotFound
...
class admin(object):
    ...
    """
    _tls_configuration = ldap3.Tls(validate=ssl.CERT_NONE,
    ...
    def __init__(self,ldap_admin, admin_pwd,ldap_server_list):
        ...
        for srv in ldap_server_list:
            srv_obj.append(ldap3.Server(srv,use_ssl=True, get_info=ldap3.ALL,
                tls=self._tls_configuration))
        sever_pool = ldap3.ServerPool(srv_obj,pool_strategy=ldap3.ROUND_ROBIN,
            active=True)
        self.conn = ldap3.Connection(sever_pool, user=ldap_admin,
            password=admin_pwd)
        self.conn.start_tls()
        self.conn.bind()
    ...
    def create_user(self, user_instance, new_password):
        """ Creates a user in ldap.
            ...
            Raises:
                ...
                type.
        """
        if isinstance(user_instance, user.user) or \
            issubclass(user_instance.__class__,user.user):
            attrs={}
            ...
            if self.conn.result['result']!=0:
                raise OperationFailure('Can not create user: '+
            ...
            raise WrongParam('Type mismatch for user_instance '+
    ...
    def update_user(self, from_user_inst, to_user_inst,force=False):
        ...
        """
        diff_attrs=from_user_inst.diff_ldap_attrs_by_categories(to_user_inst,
            force)
        #print diff_attrs
        self.conn.modify(from_user_inst.get_dn(), diff_attrs )
        if self.conn.result['result']!=0:
            raise OperationFailure('Can not update user: '+
                    '{user}. Server meseage: {desc} - {msgs}.'.format(
    ...
    def rename_user_rdn(self, from_user_inst, to_user_inst):
        ...
        if self.conn.result['result']!=0:
            raise OperationFailure('Can not rename RDN user: '+
                    '{user}. Server meseage: {desc} - {msgs}.'.format(
                    user=user_instance.brief,
                    desc=self.conn.result['description'],
                    msgs=self.conn.result['message']))
    ...
    def get_user(self, login,  user_class_name):
        ...
        resp=self.conn.search('dc=unn,dc=global',
            '(sAMAccountName={login})'.format(login=login) ,
            attributes=list(cls._FIELD_MAP.values()))

        if resp==False:
            raise NotFound('User login: {login}, uid: {uid} not found.'.format(
                login=login,uid=uid))
        for cls_attr, ldap_attr in cls._FIELD_MAP.items():
            if ldap_attr in self.conn.entries[0]:
                ...
                cls_vl[cls_attr]= '' if l3_attr.value is None else l3_attr.value
        usr=cls(**cls_vl)
    ...
    def get_some_user(self, login):
        """ Find a user in ldap.
            ...
            Raises:
                NotFound: User is not found.
        """
        resp=self.conn.search(search_base=self.ldap_domain_name,
            search_filter='(sAMAccountName={login})'.format(login=login) ,
            attributes=ldap3.ALL_ATTRIBUTES)

        if resp==False:
            raise NotFound('User login: {login} not found.'.format(login=login))
        else:
            try:
            ...
            for cls_attr, ldap_attr in cls._FIELD_MAP.items():
                if ldap_attr in self.conn.entries[0]:
                    l3_attr=getattr(self.conn.entries[0],ldap_attr)
                    cls_vl[cls_attr]= '' if l3_attr.value is None \
                        else l3_attr.value
            usr=cls(**cls_vl)
            return usr
    ...
    def search_users(self,ldap_filter, base=''):
        ...
        resp=self.conn.search(search_base=base,
                ...
                if 'attributes' in itm:
                    ...
                    for cls_attr, ldap_attr in cls._FIELD_MAP.items():
                        if ldap_attr in itm['attributes']:
                            ...
                            if  isinstance(l3_attr,list) and cls_attr!='memberof':
                                cls_vl[cls_attr]= l3_attr[0]
                            else:
                                cls_vl[cls_attr]= l3_attr

                    if not isinstance(itm['attributes']['distinguishedName'],str):
    ...
    def _add_users_to_groups(self, members_dn, groups_dn):
        return ad_add_members_to_groups(self.conn, members_dn, groups_dn,
                fix=False)
    ...
    def add_users_to_groups(self, user_instance_list, group_instance_list):
        ...
        return ad_add_members_to_groups(self.conn, members_dn, group_list,
                fix=True)

    def _remove_users_from_groups(self, members_dn, groups_dn):
        return ad_remove_members_from_groups(self.conn, members_dn, groups_dn,
                fix=False)

    def remove_users_from_groups(self, user_instance_list, group_instance_list):
        ...
        members_dn=[u.get_dn() for u in user_instance_list]
        group_list=[g.get_dn() for g in group_instance_list]
        return ad_remove_members_from_groups(self.conn, members_dn, group_list,
                fix=True)
    ...
    def _get_group(self, group_name):
        ...
        resp=self.conn.search(search_base=self.ldap_domain_name,
            search_filter='(&(objectClass=group)(cn={grp}))'.format(
                grp=group_name), attributes=attributes)
    ...
    def get_group(self, group_name, grp_cls_name):
        ...
        if resp==False:
            ...
            cls_vl={'name':group_name, 'org_unit':ou,
    ...
    def search_groups(self, ldap_filter, grp_cls_name, base=''):
        ...
        resp=self.conn.search(search_base=base,
            search_filter=ldap_filter , search_scope=ldap3.SUBTREE,
            attributes=ldap3.ALL_ATTRIBUTES)
    ...
    def search_groups_p(self, ldap_filter, grp_cls_name, base=''):
        ...
        resp=self.conn.extend.standard.paged_search(search_base=base,
                                        search_filter=ldap_filter ,
                                        attributes=[ldap3.ALL_ATTRIBUTES,
...
##        pass
    ...
    def _create_group(self, group_name, base, description=''):
        ...
        if self.conn.result['result']!=0:
            raise OperationFailure('Can not create group: '+
        ...
        """ Creates a group in ldap.
            ...
            Raises:
                ...
                WrongParam: The user_instance parameter is not of the correct
                    type.
        """
        if isinstance(group_instance, group.group) or \
            issubclass(group_instance.__class__,group.group):
            attrs={}
            ...
            raise WrongParam('Type mismatch for group_instance '+
    ...
    def _create_ou(self, ou_name, base, description=''):
        ...
        self.conn.add(dn, object_class=['organizationalUnit','top'],
            attributes=attrs)
        if self.conn.result['result']!=0:
            raise OperationFailure(
                    'Can not create OU: '+
    ...
    def create_ou(self, ou_instance):
        """ Creates an organizational unit.
            ...
            Raises:
                OperationFailure: The operation failed.
        """
        if isinstance(ou_instance, ou.ou) or \
            issubclass(ou_instance.__class__,ou.ou):
            attrs={}
            ...
            raise WrongParam('Type mismatch for ou_instance '+

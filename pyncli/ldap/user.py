...
class user(protouser.protouser, metaclass=CleanSetAttrMeta):
    ...
    _LDAP_LIMITS = {
                ...
                'login':{'min':1,'max':64,'fail_min':True,'fail_max':True}, # 20 https://msdn.microsoft.com/en-us/library/ms679635.aspx
                'org_unit':{'min':1,'max':1024,'fail_min':True,'fail_max':True},
                'surname':{'min':0,'max':64,'fail_min':False,'fail_max':False},
                'first_name':{'min':0,'max':64,'fail_min':False,'fail_max':False},
                'middle_name':{'min':0,'max':64,'fail_min':False,'fail_max':False},
                'company':{'min':0,'max':64,'fail_min':False,'fail_max':False},
                'department':{'min':0,'max':64,'fail_min':False,'fail_max':False},
                'division':{'min':0,'max':256,'fail_min':False,'fail_max':False},
                'position':{'min':0,'max':128,'fail_min':False,'fail_max':False},
                'mail':{'min':0,'max':256,'fail_min':False,'fail_max':False},
                'mobile':{'min':0,'max':64,'fail_min':False,'fail_max':False},
                'other_mailbox':{'min':0,'max':64,'fail_min':False,'fail_max':False},
                'other_mobile':{'min':0,'max':64,'fail_min':False,'fail_max':False},
                'comment':{'min':0,'max':1024,'fail_min':False,'fail_max':False},
                'employee_type':{'min':0,'max':256,'fail_min':False,'fail_max':False},
                'description':{'min':0,'max':1024,'fail_min':False,'fail_max':False},
                'full_name':{'min':0,'max':256,'fail_min':False,'fail_max':False},
                'initials':{'min':0,'max':6,'fail_min':False,'fail_max':False},
                'dn':{'min':1,'max':4096,'fail_min':True,'fail_max':False}, #max size not found :(
                'principal_name':{'min':0,'max':1024,'fail_min':False,'fail_max':False},
    }
    _DEFAULT_SORT_ORDER=['login','uid',
                            'surname',
                            'first_name',
                            'middle_name',
                            'position',
    ...
    def __init__(self,login, uid='', org_unit='OU=test,DC=example,DC=com',
                surname='', first_name='', middle_name='',
                company='', department='', division='', position='',
                mail='', mobile='',
                other_mailbox='0', other_mobile='0',
        ...
        """constructor
            ...
            Raises:
                ...
                type.
        """
        attr_set=['uid','surname','first_name','middle_name','company',
                'department','division','position','mail','mobile',
                'other_mailbox','other_mobile','comment','employee_type','description']
        ...
        if isinstance(org_unit, str):
            ...
            try:
                ...
                raise WrongParam('Unicode string expected (org_unit), conversion fails.')
        ...
        self.principal_name=self.check_length('principal_name', self.login+'@'+self.get_domain() )
    ...
    def __setattr__(self, name, value):
        ...
        if name=='middle_name':
            ...
            super(user,self).__setattr__( 'initials', self.check_length('initials', self.get_initials() ) )
            super(user,self).__setattr__( 'full_name', self.check_length('full_name', self.get_fullname() ) )
        if name=='org_unit':
            super(user,self).__setattr__( name, self.check_length( name, value) )
            super(user,self).__setattr__( 'dn', self.check_length( 'dn', self.get_dn() ) )
            super(user,self).__setattr__( 'principal_name',
                self.check_length('principal_name', self.login+'@'+self.get_domain() ) )
        if name=='surname':
            super(user,self).__setattr__( name, self.check_length( name, value) )
            super(user,self).__setattr__( 'full_name', self.check_length( 'full_name', self.get_fullname() ) )
        if name=='first_name':
    ...
    def get_domain(self):
        ...
        beg=ou_dn.find('dc=')
        if beg==-1:
            raise WrongParam('Parameter org_unit has wrong format. Domain name part not found.')
        dom=ou_dn[beg:]
        dom=dom.replace('dc=','')
        dom=dom.replace(',','.')
        return dom
    ...
    def get_ldap_attrs(self):
        ...
        for a in list(self._FIELD_MAP.keys()):
            if a== 'acc_control':
                _attrs[self._FIELD_MAP[a]]= str( uac.get_control( self.__getitem__(a) ) )
            else:
            ...
            for (key, value) in list(self.__dict__.items()):
                if self.__getitem__(key)!=instance.__getitem__(key):
                    if key in list(self._FIELD_MAP.keys()):
                        if key == 'acc_control':
                            ret[self._FIELD_MAP[key]]=str( uac.get_control( instance.__getitem__(key) ) )
                        else:
                ...
                if self.__getitem__(key)!=instance.__getitem__(key):
                    if key in list(self._FIELD_MAP.keys()):
                        ...
                        if self.__getitem__(key)=='':
                            OP='MODIFY_ADD'
                            if key == 'acc_control':
                                VL.append(int( uac.get_control( instance.__getitem__(key) ),16 ))
                            else:
    ...
    def brief(self):
        ...
        """
        return "{fio} (l:{login}|u:{uid})".format(fio=self.full_name,
                                               login=self.login,
                                               uid=self.uid)
    @classmethod
    ...
    def get_sql_insert(self, table_name):
        ...
        for itm in list(self._LDAP_LIMITS.keys()):
            ...
            vals+="\'{val}\', ".format(val= utill.sqllite_quote_ap( getattr( self, itm ) ) )
        ins+="acc_control , enabled) "
        vals+="\'{acc_cont}\', \'{enabled}\');".format(acc_cont=uac.get_control( self.acc_control),enabled=self.enabled )
        return ins+vals

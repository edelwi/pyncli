...
class protouser(object):
    ...
    def __init__(self, login, **kwargs):  # uid,
        ...
        if isinstance(login, str):
            ...
            try:
                ...
                raise WrongParam(
                    "Unicode string expected (login), conversion fails."
                )
    ...
    def get_ldap_attr(self, ldap_attr):
        ...
        attr = [
            atr
            for atr, ld_atr in list(self._FIELD_MAP.items())
            if ld_atr == ldap_attr
        ][0]
        return attr
            ...
            if len(df) > 0:
                ...
                for sortedKey in field_order:
                    if sortedKey in list(df.keys()):
                        ...
                        out_line = "{item_name:<{width}}:{mark}{orig}".format(
                            item_name=itm_k,
    ...
    def show(self, field_order=[], by_ldap_attf=False, print_out=False):
        ...
        if by_ldap_attf:
            max_descr_len = len(max(list(self._FIELD_MAP.values()), key=len))
            df = {
                self._FIELD_MAP[key]: getattr(self, key)
                for key in list(self._FIELD_MAP.keys())
            }
            if len(df) > 0:
                out_line = "=" * 80
                if print_out:

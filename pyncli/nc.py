...
def get_quote_bytes(quote_str):
    ...
    if match:
        ...
        mul = (
            match.groupdict()["multiplier"].lower()
            if "multiplier" in match.groupdict()
            else ""
        )
        m = 0
        if mul == "g":
...
def new_groupfolder(args):
    ...
    if "groupfolders" not in cloud._apps:
        logger.warning(
            "Your server does not support group folder functionality."
        )
        return
    ncgroup = ocs.Group(share_name, permissions)
    gf = ocs.GroupFolder(
        id=0,
        mount_point="{pref}{gr}{suf}".format(
            pref=PREF, suf=SUF, gr=share_name
        ),
        groups=[ncgroup],
        quota=quota,
    )
        ...
        for usr in user_list:
            if "fullname" in usr:
                u = adm.search_users_p(
                    "(&(objectClass=user)(displayname={fullname})(!(UserAccountControl:1.2.840.113556.1.4.803:=2)))".format(
                ...
                if len(u) == 1:
                    ...
                    logger.warning(
                        'User "{u}" not found. Skip it.'.format(
                            u=usr["fullname"]
                        )
                    )
                else:
                    logger.warning(
                        'Searching for user "{u}" returned an ambiguous result: {r}. Set the user by login.'.format(
                ...
                if len(u) == 1:
                    ...
                    logger.warning(
                        'User "{u}" not found. Skip it.'.format(u=usr["login"])
                    )
                else:
                    logger.warning(
    ...
    if ldap_ok:
        ...
        for usr in user_list:
            if "fullname" in usr:
                logger.warning(
                    "The directory is not available. I can not add a user by their full name. Skipping user {u}.".format(
                        u=usr["fullname"]
                    )
                )
            elif "login" in usr:
    ...
    if ldap_ok:
        ldap_grp = group.group(
            ...
            description="{descr} {gr}".format(
                descr=Config.LDAP_NEW_GROUP_DESCRIPTION, gr=share_name
            ),
        )
        adm.create_group(ldap_grp)

        share_grp = adm.get_group(
            "{lpr}{gr}{lsu}".format(lpr=LPR, lsu=LSU, gr=share_name), "group"
        )
        if LPARENT:
...
def new_group(args):
    ...
    if ldap_ok:
        ldap_grp = group.group(
            ...
            org_unit=Config.LDAP_SEARCH_FOR_GROUPS,
            description="{descr} {gr}".format(
                descr=Config.LDAP_NEW_GROUP_DESCRIPTION, gr=group_name
            ),
        )
        adm.create_group(ldap_grp)
...
def new_user(args):
    ...
    try:
        ...
        logger.error(
            "Can not create user {u}: {err}".format(u=login, err=e.value)
        )
        return
    params = vars(args)
...
def del_group_member(args):
    ...
    if login and ldap_ok:
        ...
        if len(u) == 1:
            ...
            logger.warning('User "{u}" not found.'.format(u=fullname))
            return
        else:
            logger.warning(
                'Searching for user "{u}" returned an ambiguous result: {r}. Set the user by login.'.format(
        ...
        logger.warning(
            "The directory is not available. I can not remove a user by their full name. Skipping user {u}.".format(
                u=fullname
            )
        )
        return
        ...
        logger.warning(
            'User is not defined. Login: "{l}", Fullname: "{f}"'.format(
                l=login, f=fullname
            )
        )
        return
...
def del_group(args):
    ...
    if ldap_ok:
        ...
        try:
            ...
            logger.error(
                "Could not delete ldapgroup {g}. Error: {e}".format(
                    g=ldap_grp.brief, e=e.value
                )
            )
            ...
            for grp in gf.groups:
                if grp.group_id == group_name:
                    cloud.revoke_access_to_group_folder(
                        gfolder_id=gf.id, group_id=group_name
                    )
                    return
    else:
        logger.warning(
            "Group folder {gp} with group {grp} not found.".format(
...
def del_groupfolder(args):
    ...
    for gf in group_folders:
        if gf.mount_point == PREF + groupfolder_name + SUF:
            ...
            for grp in gf.groups:
                ...
                if ldap_ok:
                    ...
                    logger.debug(
                        "Trying to delete group: {gp}.".format(gp=ldap_grp)
                    )
                    try:
                        adm.delete_group(ldap_grp)
        ...
        logger.warning(
            "Group folder {gp} not found.".format(gp=groupfolder_name)
        )
...
def add_group(args):
    ...
    if not gf:
        logger.error(
            "The specified group folder {gp} is not found!".format(
                gp=group_folder
            )
        )
        exit(0)
    cloud.grant_access_to_group_folder(gfolder_id=gf.id, group_id=group_name)
    cloud.set_group_folder_group_permissions(
        gfolder_id=gf.id, group_id=group_name, permissions=permissions
    )
    ...
    for usr in user_list:
        if "fullname" in usr and ldap_ok:
            ...
            if len(u) == 1:
                ...
                logger.warning(
                    'User "{u}" not found. Skip it.'.format(u=usr["fullname"])
                )
            else:
                logger.warning(
                    'Searching for user "{u}" returned an ambiguous result: {r}. Set the user by login.'.format(
...
def get_groupfolder(args):
    ...
    for gf in group_folders:
        ...
        logger.warning(
            "Group folder {gp} not found.".format(gp=group_folder_name)
        )
...
def set_groupfolder_name(args):
    ...
    for gf in group_folders:
        if gf.mount_point == PREF + group_folder_name + SUF:
            cloud.rename_group_folder(
                gf.id, PREF + new_group_folder_name + SUF
            )
            return
    else:
        logger.warning(
...
def command_line():
    ...
    user_quota_help_message = (
        "Sets a quota for a user. "
        + "You can use the modifiers k, m, g, t. For example 5g."
    )
    usersfile_help_message = (
        "The path to the file with users (by full name or login in the line)."
    )
    groupfolder_help_message = "The name of the group folder."
    groupname_help_message = (
        "The name of the group in the cloud (without prefix and suffix)."
    )
    ...
    permissions_help_message = """Permissions can be set to the following values:
        r - read permission
...
"""
    ...
    parser.add_argument(
        "--version",
        action="version",
        version="{pr} {ver}".format(pr=prog_name, ver=prog_ver),
    )

    subparsers = parser.add_subparsers(dest="top")

    new__group = subparsers.add_parser(
        "new",
        description=textwrap.dedent(
            """create:
...
"""
    ...
    del__group = subparsers.add_parser(
        "del",
        description=textwrap.dedent(
            """delete:
...
"""
    ...
    get__group = subparsers.add_parser(
        "get",
        description=textwrap.dedent(
            """get:
...
"""
    ...
    add__group = subparsers.add_parser(
        "add",
        description=textwrap.dedent(
            """add:
...
"""
    ...
    set__group = subparsers.add_parser(
        "set",
        description=textwrap.dedent(
            """set:
...
"""
    ...
    usr_new.add_argument(
        "-e", "--email", action="store", type=str, help=email_help_message
    )
    usr_new.add_argument(
        "-i",
        "--displayname",
    ...
    usr_new.add_argument(
        "-q", "--quota", action="store", type=str, help=user_quota_help_message
    )
    usr_new.add_argument(
        "-m", "--phone", action="store", type=str, help=phone_help_message
    )
    usr_new.add_argument(
        "-a", "--address", action="store", type=str, help=address_help_message
    )
    usr_new.add_argument(
        "-w", "--website", action="store", type=str, help=website_help_message
    )
    usr_new.add_argument(
        "-t", "--twitter", action="store", type=str, help=twitter_help_message
    )
    usr_new.add_argument(
        "-g",
        "--group",
    ...
    grp_mem_del.add_argument(
        "-l", "--login", action="store", type=str, help=login_help_message
    )
    ...
    usrs_get.add_argument(
        "-l", "--limit", action="store", type=int, help=usrs_limit_help_message
    )
    usrs_get.add_argument(
        "-o",
        "--offset",
    ...
    usr_add.add_argument(
        "-n", "--name", action="store", type=str, help=fullname_help_message
    )
    usr_add.add_argument(
        "-g",
        "--group",
